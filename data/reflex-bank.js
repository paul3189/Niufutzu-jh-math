...
(function () {
  ...
  var GEO = [
    ...
    ["similar.sim", 9, "兩個四邊形的四個角都對應相等，一定相似嗎？",
      "不一定（反例：矩形與正方形）", ["一定相似（AA）", "一定不相似", "只有都是直角時相似"],
      "多邊形要「對應角相等」<b>且</b>「對應邊成比例」才相似；只看角不夠。AA 只對三角形成立"],
    ["similar.sim", 9, "兩個四邊形的四邊都對應成比例，一定相似嗎？",
      "不一定（反例：菱形與正方形）", ["一定相似（SSS）", "一定不相似", "比值是 1 時才相似"],
      "邊成比例但角可以不同（菱形壓扁了四邊還是等長）；SSS 相似只對三角形成立"],
    ["similar.sim", 9, "兩個多邊形要「相似」，需要什麼條件？",
      "對應角相等，且對應邊成比例", ["對應角相等就夠", "對應邊成比例就夠", "面積成比例"],
      "兩個條件缺一不可；只有三角形可以省成 AA、SAS、SSS"],
  ...
  /* ══════════════ 依 108 課綱條文補齊 ══════════════
   * 逐條核對學習內容後，補上原本漏掉的：空間中的線與平面（S-9-12；歪斜只在學習內容出現、
   * 學習表現 s-IV-15 沒有，所以標成補充題）、點與圓的位置關係（S-9-7）、
  ...
  GEO = GEO.concat([
    ...
    ["solid.space", 9, "長方體中，兩條稜線「不平行、也不相交」，這種關係叫什麼？",
      "歪斜", ["垂直", "平行", "重合"],
      "空間中兩直線有三種關係：平行、相交、歪斜；歪斜的兩條線不在同一個平面上", true],
    ["solid.space", 9, "長方體 $ABCD$-$EFGH$ 中（$A$ 在 $E$ 的正上方，依此類推），$\\overline{AB}$ 與 $\\overline{CG}$ 的關係是？",
      "歪斜", ["平行", "垂直相交", "相交但不垂直"],
      "$\\overline{AB}$ 在上底面、$\\overline{CG}$ 是直立的稜線，兩者沒有交點也不平行 → 歪斜", true],
    ["solid.space", 9, "長方體直立的稜線 $\\overline{AE}$ 與底面 $EFGH$ 的關係是？",
      "垂直", ["平行", "相交但不垂直", "只與 $\\overline{EF}$ 垂直"],
    ...
    ["solid.space", 9, "正四面體 $ABCD$ 的稜線 $\\overline{AB}$ 與 $\\overline{CD}$（沒有共同頂點）是什麼關係？",
      "歪斜", ["平行", "相交", "在同一平面上"],
      "沒有共同頂點的兩條稜（對稜）不相交也不平行；正四面體有 3 組對稜", true],
